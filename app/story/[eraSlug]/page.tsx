...
import { useProgress } from "@/hooks/useProgress"; // Import Hook theo dõi tiến độ

// Định nghĩa thứ tự các kỷ nguyên để điều hướng
...
export default function EraPage() {
  ...
  // Lấy dữ liệu tiến độ từ hook
  const { completedStories, isFullyUnlocked } = useProgress();

  // Lấy dữ liệu theo mốc thời gian
  const eraData = ERAS_DATA[eraSlug];

  // Tính toán kỷ nguyên Trước và Sau để làm nút điều hướng
  ...
  // Nếu nhập sai đường dẫn hoặc dữ liệu không tồn tại
  if (!eraData) return <div className="text-white p-10">Không tìm thấy dữ liệu kỷ nguyên này.</div>;

  return (
    <main className="relative min-h-screen bg-stone-900 overflow-hidden font-sans">
      
      {/* 1. NÚT QUAY LẠI TIMELINE (Góc trên trái) */}
      ...
      {/* 2. BACKGROUND (Thay đổi theo dữ liệu) */}
      <motion.div 
        key={eraSlug} // Key thay đổi để kích hoạt animation khi đổi trang
      ...
      <div className="absolute top-10 left-0 right-0 text-center z-10 pointer-events-none">
        ...
        >
            ...
            <h1 className="text-5xl md:text-7xl text-amber-500 font-serif font-bold drop-shadow-lg">
                {eraData.title}
            </h1>
        </motion.div>
      </div>

      {/* 4. NÚT ẨN (SECRET BUTTON) - CHỈ HIỆN KHI XEM HẾT TẤT CẢ */}
      <AnimatePresence>
        {isFullyUnlocked && (
          <motion.div
      ...
      {/* 5. CÁC ĐIỂM MỐC (Render động từ dữ liệu) */}
          ...
          return (
            ...
            >
              <Link href={`/story/${eraSlug}/${story.id}`}>
                ...
                >
                  {/* Nếu xong: Xanh lá, Chưa xong: Vàng */}
                  ...
                  <div className={`relative w-6 h-6 rounded-full border-4 border-black transition-transform group-hover:scale-125 
                    ${isDone 
                      ? 'bg-green-500 shadow-[0_0_15px_rgba(34,197,94,0.8)]' // Style đã xong
                      : 'bg-amber-400 shadow-[0_0_15px_rgba(251,191,36,0.8)]' // Style chưa xong
                    }`} 
                  />
                    ...
                    {hoveredStory === story.id && (
                      ...
                      >
                        <div className="flex justify-between items-start">
                           <h3 className="text-amber-400 text-lg font-serif font-bold mb-1">{story.title}</h3>
                           {/* Dấu tích xanh nhỏ trong tooltip nếu đã xong */}
                        ...
                        <p className="text-stone-300 text-sm">{story.description}</p>
                      </motion.div>
                    )}
                  </AnimatePresence>
                </motion.div>
        ...
        <Link href={`/story/${prevEraSlug}`}>
          ...
          >
            ...
            <div className="ml-4 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                ...
                <p className="text-lg font-serif font-bold text-white">{ERA_NAMES[prevEraSlug]}</p>
            </div>
          </motion.div>
        </Link>
      )}
        ...
        <Link href={`/story/${nextEraSlug}`}>
          ...
          >
            ...
            <div className="mr-4 opacity-0 group-hover:opacity-100 transition-opacity duration-300 text-right">
                ...
                <p className="text-lg font-serif font-bold text-white">{ERA_NAMES[nextEraSlug]}</p>
            </div>
          </motion.div>
        </Link>
      )}
